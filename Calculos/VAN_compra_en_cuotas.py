...
def procesar():
    ...
    if d1>0 and d2>0 and d3>0 and d4>0:
        flujo_fondos.append(d1)
        contador=0
        while contador<d3:
            flujo_fondos.append(d2*-1)
            contador+=1
        NPV=0
        ...
        if d1==0 or d2==0 or d3==0 or d4==0:
            ...
            recomend.append('Conviene comprar en efectivo')
        else:
            recomend.append('Conviene comprar en cuotas')  
    else:
        if float(d1)<=0:
            resultado.append('El importe de compra es incorrecto.')
        else:
            pass 
        if float(d2)<=0:
            resultado.append('El importe valor de cuota es incorrecto.')
        else:
            pass 
        if float(d3)<=0: 
            resultado.append('El valor cantidad de cuotas es incorrecto.')
        else:
            pass 
        if float(d4)<=0:
...
l_comp_efvo=Label(root, text="Valor compra en efectivo: ")
l_comp_efvo.grid(row=2, column=1, sticky="w", padx="10", pady="10")

l_val_cuotas=Label(root, text="Valor de las cuotas: ")
l_val_cuotas.grid(row=3, column=1, sticky="w", padx="10", pady="10")

l_cant_cuotas=Label(root, text="Cantidad de cuotas: ")
