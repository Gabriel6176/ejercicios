...
def procesar():
    ...
    with open(APP_PATH+'\\'+d1+'.txt', 'r', encoding='ANSI', newline='\n') as f:
        ...
        for row in txt_line:
            contador=1
            print(row)
            #dt1 = re.search('(\d\d\d\d\d\d\d\d)', linea).group(1)
            #nombro los campos que voy a insertar sin indicar el de ID AUTOINCREMENT
            cursor.execute('INSERT INTO '+d3+' (FECHA1, FECHA2, FECHA3, CUIT, D1, D2, D3, D4, D5, D6, D7, NOMBRE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', row)
            contador+=1
    con.commit()
    con.close()
