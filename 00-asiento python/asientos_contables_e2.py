...
input_file = r'C:\Users\PC\Desktop\B2021\Sumas y Saldos 2021_5.xlsx'
output_file = r'C:\Users\PC\Desktop\B2021\Sumas y Saldos 2021_listo_5.xlsx'
...
# Lista de numeros que quiero agregar el texto de Nota-ASiento + Texto campo #ORDEN_PAGO
lista_texto_op=[50.14, 50.24, 50.34, 50.44, 56.4, 80.2, 80.3, 80.4, 89.3, 89.4, 95.94, 96.3,]
...
def account_entry_1(row, local_vars, asiento_prefix, fecha_prefix, cta_cont_prefix, detalle_prefix, debe_prefix, haber_prefix, nota_prefix,
    ...
    if pd.isna(local_vars['orden_pago']):
        row[nota_prefix] = local_vars['nota_a']
    else:
        row[nota_prefix] = local_vars['nota_a'] + (str(local_vars['orden_pago']) if local_vars['cod'] in lista_texto_op else '')
    ...
    if pd.isna(local_vars['orden_pago']):
        ...
        row[nota_2_prefix] = local_vars['nota_a'] + (str(local_vars['orden_pago']) if local_vars['cod'] in lista_texto_op else '')
...
def process_row(row):
    ...
    # Conjuntos de códigos para la lógica condicional
    cod_1_set = {19.4, 20.13, 22.1, 22.2, 22.3, 22.4, 25.1, 25.2, 25.3, 25.4, 30.31, 30.34, 30.43, 50.14, 50.24, 50.34, 50.44, 50.54, 
                 51.4, 51.94, 52.4, 52.94, 56.4, 60.2, 81.3, 81.4, 81.6, 89.3, 89.4, 96.3, 97.3}
    ...
    if local_vars['cod'] in cod_1_set:
        ...
        account_entry_1(row, local_vars, '#ASIENTO_1', '#FECHA_1', '#CTA_CONT_1', '#DETALLE_1', '#DEBE_1', '#HABER_1', '#NOTA_1',
                        '#ASIENTO_2', '#FECHA_2', '#CTA_CONT_2', '#DETALLE_2', '#DEBE_2', '#HABER_2', '#NOTA_2')
        account_entry_2(row, local_vars, '#ASIENTO_3', '#FECHA_3', '#CTA_CONT_3', '#DETALLE_3', '#HABER_3', '#DEBE_3', '#NOTA_3',
                        '#ASIENTO_4', '#FECHA_4', '#CTA_CONT_4', '#DETALLE_4', '#HABER_4', '#DEBE_4', '#NOTA_4',
                        '#PART_PRESU', '#CTA_PP', '#DET_PP')
    return row
...
print("--------------------ESTE ES EL DATA FRAME PROCESSED---PREVIO EXCLUDE---------------")
print(df_processed.head())

# Exclude columns before writing to Excel
columns_to_exclude = [
    '#CTA', '#NUM', '#FECHA', '#DIA', '#MES', '#COD', '#NOTA', '#ORDEN_PAGO', '#PROVEEDOR', '#PART_PRESU', '#SALDO_DEUDOR', '#SALDO_ACREEDOR', '#COD_A', 
...
# Set the option to display all columns
pd.set_option('display.max_columns', None)
print("--------------------ESTE ES EL DATA FRAME PROCESSED------------------")
print(df_processed.head())
...
# Set the option to display all columns
pd.set_option('display.max_columns', None)
print("--------------------ESTE ES EL DATA FRAME FINAL------------------")
print(df_final.head())
...
# Set the option to display all columns
pd.set_option('display.max_columns', None)
print("--------------------ESTE ES EL DATA FRAME DF-----------------")
print(df.head())
