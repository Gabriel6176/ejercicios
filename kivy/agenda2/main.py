...
class MainWid(ScreenManager):
    def __init__(self,**kwargs):
        ...
        self.DB_PATH = self.APP_PATH+'/my_agenda2.db'
...
class DataBaseWid(BoxLayout):
    ...
    def check_memory(self):
        ...
        cursor = con.cursor()
        cursor.execute('select ID, Nombre, Apellido, Telefono, Diagnostico, Mama, Papa from Pacientes')
        for i in cursor:
            ...
            r6 = 'Papa: '+i[6]
            wid.data_id = str(i[0])
            #solo elijo los campos que quiero ver
            wid.data = r1+r2+r3
...
class UpdateDataWid(BoxLayout):
    ...
    def check_memory(self):
        con = sqlite3.connect(self.mainwid.DB_PATH)
        cursor = con.cursor()
        s = 'select Nombre, Apellido, Telefono, Diagnostico, Mama, Papa from Pacientes where ID='
        cursor.execute(s+self.data_id)
        for i in cursor:
            self.ids.ti_nombre.text = i[0]
            self.ids.ti_apellido.text = i[1]
            self.ids.ti_telefono.text = str(i[2])
            self.ids.ti_diagnostico.text = i[3]
            self.ids.ti_mama.text = i[4]
            self.ids.ti_papa.text = i[5]
        con.close()
    ...
    def update_data(self):
        ...
        d5 = self.ids.ti_mama.text
        d6 = self.ids.ti_papa.text
        a1 = (d1,d2,d3,d4,d5,d6)
        s1 = 'UPDATE Pacientes SET'
        #poniendo las dobles comillas al%s le digo que es un texto por eso telefono no las tiene
        s2 = 'Nombre="%s",Apellido="%s",Telefono=%s,Diagnostico="%s",Mama="%s",Papa="%s"' % a1
...
class InsertDataWid(BoxLayout):
    ...
    def insert_data(self):
        ...
        d6 = self.ids.ti_mama.text
        d7 = self.ids.ti_papa.text
        a1 = (d1,d2,d3,d4,d5,d6,d7)
        s1 = 'INSERT INTO Pacientes(ID, Nombre, Apellido, Telefono, Diagnostico, Mama, Papa)'
