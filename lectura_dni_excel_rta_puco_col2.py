...
CHUNK_SIZE = 1000
NUEVO_CICLO = 100000
EXCEL_NAME = "DNI_prestaciones_2024.xlsx"
            ...
            if result:
                for item in result:
                    if item[0] not in processed_nros_documentos:  # Check if NroDocumento is not processed before
                        ...
                        # Write data to Excel
                        ws.cell(row=row, column=2, value=item[1])  # Column E: NOMBRE_PACIENTE
                        ws.cell(row=row, column=3, value=item[2])  # Column F: OBRAS_SOCIAL
                        ws.cell(row=row, column=4, value=item[3])  # Column G: SIGLAS
                        ws.cell(row=row, column=5, value=numero_documento)  # Column H: numero DNI
                        cont += 1
